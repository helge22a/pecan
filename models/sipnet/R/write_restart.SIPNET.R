...
write_restart.SIPNET <- function(outdir, runid, start.time, stop.time, settings, new.state,
 ...
 if ("AbvGrndWood" %in% variables) {
  ...
  if("litter_mass_content_of_water" %in% variables){
    ...
    if (new.state$litter_mass_content_of_water < 0) analysis.save[[length(analysis.save)]] <- 0
    if(new.state$litter_mass_content_of_water > 1) analysis.save[[length(analysis.save)]] <- 1
