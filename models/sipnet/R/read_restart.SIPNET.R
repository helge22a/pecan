...
read_restart.SIPNET <- function(outdir, runid, stop.time, settings, var.names, params) {
  ...
  if ("AbvGrndWood" %in% var.names) {
    ...
    params$restart["fine_root_carbon_content"] <- ens$fine_root_carbon_content[last]
    params$restart["coarse_root_carbon_content"] <- ens$coarse_root_carbon_content[last]
  }else{
